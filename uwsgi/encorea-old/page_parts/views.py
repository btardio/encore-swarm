...
def view_makesearchrequest(request):
    ...
    for server in settings.SOLR_SERVERS:
        ...
        requeststr = 'http://' + server + '/solr/encorea/select?q=*:*&fl=partnumber,year,make,model,engineliters,enginecylinder,enginecc,engineblock,product&rows=10000'
...
def view_executepartdetailsearch(request):
    ...
    for server in settings.SOLR_SERVERS:
        ...
        requeststr = "http://" + server + "/solr/encorea/select?q=partnumber:\"" + partnumber + "\"&fl=partnumber,year,make,model,engineliters,enginecylinder,enginecc,engineblock,product&rows=1"
...
def view_javascript_error(request):
    ...
    if 'errorcode' in request.GET and len(request.GET['errorcode']) == 3:
        if request.GET['errorcode'] == '100':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "yearfaceted"} failed.')
            
        elif request.GET['errorcode'] == '101':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "makefaceted"} failed.')

        elif request.GET['errorcode'] == '102':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "modelfaceted"} failed.')
    
        elif request.GET['errorcode'] == '103':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "enginelitersfaceted"} failed.')   
    
        elif request.GET['errorcode'] == '104':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "enginecylinder"} failed.')   

        elif request.GET['errorcode'] == '105':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "enginecc"} failed.')   
            
        elif request.GET['errorcode'] == '106':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "engineboreinch"} failed.')   

        elif request.GET['errorcode'] == '107':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "enginestrokeinch"} failed.')   

        elif request.GET['errorcode'] == '108':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q=*:*&rows=0&facet=true", {"facet.field": "productfacet"} failed.')  

        elif request.GET['errorcode'] == '109':
            jslogger.error('http://127.0.0.1/solr/encorea/select?q= Search in makesearchrequest.')  
